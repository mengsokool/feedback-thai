import { analyzeThroughLLM } from "./llm";
...
export function simplifyFeedback(
  sSenseResult: SSenseItem[]
): SimplifiedReview[] {
...
export function generateStats(reviews: SimplifiedReview[]) {
  ...
  const averageScore =
    reviews?.reduce((acc, r) => acc + r.score, 0) / totalReviews;

  // Collect positive and negative keywords and count frequencies separately
  const positiveKeywordFrequency = reviews
  ...
  const negativeKeywordFrequency = reviews
  ...
  const keywordFrequency = reviews
    ?.flatMap((r) => r.keywords)
    .reduce((acc, keyword) => {
      acc[keyword] = (acc[keyword] || 0) + 1;
      return acc;
  ...
  return {
    ...
    averageScore,
    keywordFrequency,
    positiveKeywordFrequency,
    negativeKeywordFrequency,
  };
}
...
export function generatePrompt(
  ...
  return `
กรุณาวิเคราะห์รีวิวจากลูกค้าต่อไปนี้:

ชื่อสินค้า: ${productName}

สถิติโดยสรุป:
- จำนวนรีวิวทั้งหมด: ${stats.totalReviews}
- รีวิวเชิงบวก: ${stats.positiveReviews} (${(
    (stats.positiveReviews / stats.totalReviews) *
    100
  ).toFixed(1)}%)
- รีวิวเชิงลบ: ${stats.negativeReviews} (${(
    (stats.negativeReviews / stats.totalReviews) *
    100
  ).toFixed(1)}%)
...
1. จุดแข็งหลักของสินค้าจากรีวิว
